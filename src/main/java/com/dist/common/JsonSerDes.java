...
import com.dist.simplekafka.TopicAndPartition;
...
public class JsonSerDes {
    ...
    private static <T> T readValue(byte[] json, Class<T> clazz, ObjectMapper objectMapper) throws IOException {
        ...
        module.addKeyDeserializer(TopicAndPartition.class,
                new TopicAndPartitionKeyDeserializer());
    ...
    static class TopicAndPartitionKeyDeserializer extends KeyDeserializer {
        @Override
        public Object deserializeKey(String key, DeserializationContext ctxt) throws IOException {
            if (key.startsWith("[") && key.endsWith("]")) {
                String[] parts = key.substring(1, key.length() - 1).split(",");
                return new TopicAndPartition(parts[0], Integer.parseInt(parts[1].trim()));
            } else {
                throw new IllegalArgumentException(key + " is not valid topic partition");
            }
        }
    }
}
